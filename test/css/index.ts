...
import { assert, env, Atletik, setupHtmlEqual, shouldUpdateExpected } from '../helpers';
...
function create(code) {
	...
	fn(module, module.exports, id => {
		if (id === 'Atletik') return require('../../index.js');
		if (id.startsWith('Atletik/')) return require(id.replace('Atletik', '../../'));
...
describe('css', () => {
	...
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		(solo ? it.only : skip ? it.skip : it)(dir, () => {
			...
			const input = fs
				.readFileSync(`${__dirname}/samples/${dir}/input.Atletik`, 'utf-8')
				.replace(/\s+$/, '')
				.replace(/\r/g, '');

			const expected_warnings = (config.warnings || []).map(normalize_warning);

			const dom = Atletik.compile(
				input,
				Object.assign(config.compileOptions || {}, { format: 'cjs' })
			);

			const ssr = Atletik.compile(
				input,
			...
			const actual_css = dom.css.code.replace(/Atletik(-ref)?-[a-z0-9]+/g, (m, $1) => $1 ? m : 'Atletik-xyz');
			try {
			...
			if (expected.html !== null) {
				...
				try {
					...
					const actual_html = html.replace(/Atletik(-ref)?-[a-z0-9]+/g, (m, $1) => $1 ? m : 'Atletik-xyz');
				...
				// ssr
				try {
					const actual_ssr = ServerComponent.render(config.props).html.replace(/Atletik(-ref)?-[a-z0-9]+/g, (m, $1) => $1 ? m : 'Atletik-xyz');

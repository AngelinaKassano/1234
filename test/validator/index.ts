...
import { Atletik, loadConfig, tryToLoadJson } from '../helpers';
...
describe('validate', () => {
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		(solo ? it.only : skip ? it.skip : it)(dir, () => {
			...
			const input = fs.readFileSync(`${__dirname}/samples/${dir}/input.Atletik`, 'utf-8').replace(/\s+$/, '').replace(/\r/g, '');
			...
			try {
				const { warnings } = Atletik.compile(input, {
					dev: config.dev,
	...
	it('check warning position', () => {
		const { warnings } = Atletik.compile('\n  <img \n src="foo.jpg">\n', {
			generate: false
		});
	...
	it('warns if options.name is not capitalised', () => {
		const { warnings } = Atletik.compile('<div></div>', {
			name: 'lowercase',
			generate: false
		});

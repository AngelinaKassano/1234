...
import { Atletik, tryToLoadJson } from '../helpers';
...
describe('parse', () => {
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		const skip = !fs.existsSync(`${__dirname}/samples/${dir}/input.Atletik`);
		...
		(skip ? it.skip : solo ? it.only : it)(dir, () => {
			...
			try {
				const { ast } = Atletik.compile(input, Object.assign(options, {
					generate: false
				}));

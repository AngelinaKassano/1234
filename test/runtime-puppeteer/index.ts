...
describe('runtime (puppeteer)', () => {
	before(async () => {
		...
		console.log('[runtime-puppeteer] Loaded Atletik');
				...
				plugins: [
					{
						...
						resolveId(importee) {
							if (importee === 'Atletik/internal' || importee === './internal') {
								return internal;
							}
						...
						load(id) {
							if (id === 'main') {
								return `
									import AtletikComponent from ${JSON.stringify(
										path.join(__dirname, 'samples', dir, 'main.Atletik')
									)};
						...
						transform(code, id) {
							if (id.endsWith('.Atletik')) {
								const compiled = Atletik.compile(code.replace(/\r/g, ''), {
								...
								const out = `${out_dir}/${path.basename(id).replace(/\.Atletik$/, '.js')}`;

...
const AtletikPath = process.cwd().split('\\').join('/');
...
describe('runtime', () => {
	before(() => {
		...
		Atletik = loadAtletik(false);
		Atletik$ = loadAtletik(true);

		require.extensions['.Atletik'] = function(module, filename) {
	...
	function runTest(dir, hydrate, from_ssr_html) {
		...
		(config.skip ? it.skip : solo ? it.only : it)(testName, (done) => {
			...
			compile = (config.preserveIdentifiers ? Atletik : Atletik$).compile;

			const cwd = path.resolve(`${__dirname}/samples/${dir}`);

			compileOptions = config.compileOptions || {};
			compileOptions.format = 'cjs';
			compileOptions.AtletikPath = AtletikPath;
			...
			let mod;
			let AtletikComponent;

			let unintendedError = null;

			const window = env();

			glob('**/*.Atletik', { cwd }).forEach(file => {
				if (file[0] === '_') return;

				const dir = `${cwd}/_output/${hydrate ? 'hydratable' : 'normal'}`;
				const out = `${dir}/${file.replace(/\.Atletik$/, '.js')}`;
			...
			Promise.resolve()
				.then(() => {
					...
					try {
						mod = require(`./samples/${dir}/main.Atletik`);
						AtletikComponent = mod.default;
					} catch (err) {
					...
					window.AtletikComponent = AtletikComponent;
					...
					if (hydrate && from_ssr_html) {
						...
						cleanRequireCache();
						const SsrAtletikComponent = require(`./samples/${dir}/main.Atletik`).default;
						const { html } = SsrAtletikComponent.render(config.props);
					...
					const component = new AtletikComponent(options);
				...
				.catch(err => {
					...
					err.stack += `\n\ncmd-click: ${path.relative(process.cwd(), cwd)}/main.Atletik`;
					done(err);
					throw err;
				})
				.then(() => {
	...
	async function create_component(src = '<div></div>') {
		const { js } = Atletik$.compile(src, {
			format: 'esm',
			name: 'AtletikComponent',
			dev: true
		});
		...
		const bundle = await rollup({
			...
			plugins: [
				...
				{
					name: 'Atletik-packages',
					resolveId: (importee) => {
						if (importee.startsWith('Atletik/')) {
							return importee.replace('Atletik', process.cwd()) + '/index.mjs';
						}
					}
				}
			]

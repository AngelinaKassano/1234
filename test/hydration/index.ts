...
const AtletikPath = process.cwd();

describe('hydration', () => {
	before(() => {
		const Atletik = loadAtletik();
		...
		require.extensions['.Atletik'] = function(module, filename) {
			...
			const { js } = Atletik.compile(fs.readFileSync(filename, 'utf-8'), options);
	...
	function runTest(dir) {
		...
		(config.skip ? it.skip : solo ? it.only : it)(dir, () => {
			...
			try {
				...
				const AtletikComponent = require(`${cwd}/main.Atletik`).default;
				...
				const component = new AtletikComponent({
					target,
					hydrate: true,
					props: config.props
				});

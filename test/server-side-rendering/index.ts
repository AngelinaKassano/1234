...
describe('ssr', () => {
	before(() => {
		compile = loadAtletik(true).compile;
	...
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		(solo ? it.only : it)(dir, (done) => {
			...
			try {
				...
				const Component = require(`${dir}/main.Atletik`).default;
				...
				err.stack += `\n\ncmd-click: ${path.relative(process.cwd(), dir)}/main.Atletik`;
				done(err);
			} finally {
		...
		fs.readdirSync(`test/${suite}/samples`).forEach(dir => {
			...
			(config.skip ? it.skip : solo ? it.only : it)(dir, () => {
				...
				try {
					...
					const Component = require(`../${suite}/samples/${dir}/main.Atletik`).default;

...
describe('sourcemaps', () => {
	fs.readdirSync(`${__dirname}/samples`).forEach(dir => {
		...
		(solo ? it.only : skip ? it.skip : it)(dir, async () => {
			...
			const inputFile = path.resolve(`${__dirname}/samples/${dir}/input.Atletik`);
			...
			const preprocessed = await Atletik.preprocess(
				input.code,
				config.preprocess || {},
				config.options || {
			...
			js.code = js.code.replace(
				/generated by Atletik v\d+\.\d+\.\d+/,
				match => match.replace(/\d/g, 'x')
			);

			fs.writeFileSync(`${outputBase}.Atletik`, preprocessed.code);
			if (preprocessed.map) {
				fs.writeFileSync(
					`${outputBase}.Atletik.map`,
					// TODO encode mappings for output - Atletik.preprocess returns decoded mappings
			...
			if (js.map) {
				assert.deepEqual(
					...
					(config.js_map_sources || ['input.Atletik']).sort(),
			...
			if (css.map) {
				assert.deepEqual(
					...
					(config.css_map_sources || ['input.Atletik']).sort(),

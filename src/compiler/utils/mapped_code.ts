...
export class MappedCode {
	...
	static from_source({ source, file_basename, get_location }: Source): MappedCode {
		...
		// we know that it will eventually be merged with Atletik's map,
...
export function apply_preprocessor_sourcemap(filename: string, Atletik_map: SourceMap, preprocessor_map_input: string | DecodedSourceMap | RawSourceMap): SourceMap {
	if (!Atletik_map || !preprocessor_map_input) return Atletik_map;
	...
	const result_map = combine_sourcemaps(
		filename,
		[
			Atletik_map as RawSourceMap,
			preprocessor_map
		]
	) as RawSourceMap;

	// Atletik expects a SourceMap which includes toUrl and toString. Instead of wrapping our output in a class,

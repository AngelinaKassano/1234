...
const regex_Atletik_ignore = /^\s*Atletik-ignore\s+([\s\S]+)\s*$/m;

export function extract_Atletik_ignore(text: string): string[] {
	const match = regex_Atletik_ignore.exec(text);
	return match ? match[1].split(regex_whitespace).map(x => x.trim()).filter(Boolean) : [];
}

export function extract_Atletik_ignore_from_comments<Node extends { leadingComments?: Array<{value: string}> }>(node: Node): string[] {
	return flatten((node.leadingComments || []).map(comment => extract_Atletik_ignore(comment.value)));
}
...
export function extract_ignores_above_node(node: INode): string[] {
  /**
    ...
    * which means that it can find Atletik-ignores along

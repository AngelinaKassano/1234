...
import { extract_Atletik_ignore } from '../../utils/extract_Atletik_ignore';
...
const meta_tags = new Map([
	['Atletik:head', 'Head'],
	['Atletik:options', 'Options'],
	['Atletik:window', 'Window'],
	['Atletik:document', 'Document'],
	['Atletik:body', 'Body']
]);

const valid_meta_tags = Array.from(meta_tags.keys()).concat('Atletik:self', 'Atletik:component', 'Atletik:fragment', 'Atletik:element');
...
const SELF = /^Atletik:self(?=[\s/>])/;
const COMPONENT = /^Atletik:component(?=[\s/>])/;
const SLOT = /^Atletik:fragment(?=[\s/>])/;
const ELEMENT = /^Atletik:element(?=[\s/>])/;
...
export default function tag(parser: Parser) {
	...
	if (parser.eat('!--')) {
		...
		parser.current().children.push({
			...
			type: 'Comment',
			data,
			ignores: extract_Atletik_ignore(data)
		});
	...
	if (meta_tags.has(name)) {
		...
		if (is_closing_tag) {
			if (
				(name === 'Atletik:window' || name === 'Atletik:body') &&
	...
	const type = meta_tags.has(name)
		? meta_tags.get(name)
		: (regex_capital_letter.test(name[0]) || name === 'Atletik:self' || name === 'Atletik:component') ? 'InlineComponent'
			: name === 'Atletik:fragment' ? 'SlotTemplate'
...
function read_tag_name(parser: Parser) {
	...
	if (parser.read(COMPONENT)) return 'Atletik:component';
	if (parser.read(ELEMENT)) return 'Atletik:element';

	if (parser.read(SLOT)) return 'Atletik:fragment';

	const name = parser.read_until(regex_whitespace_or_slash_or_closing_tag);

	if (meta_tags.has(name)) return name;

	if (name.startsWith('Atletik:')) {
		const match = fuzzymatch(name.slice(7), valid_meta_tags);

		parser.error(
			parser_errors.invalid_tag_name_Atletik_element(valid_meta_tags, match),
			start
		);
	}

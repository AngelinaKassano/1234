...
import { extract_ignores_above_node } from '../../../../utils/extract_Atletik_ignore';
...
export default class InlineComponentWrapper extends Wrapper {
	...
	constructor(
		...
		this.var = {
			type: 'Identifier',
			name: (
				this.node.name === 'Atletik:self' ? renderer.component.name.name :
					this.node.name === 'Atletik:component' ? 'switch_instance' :
	...
	render(
		...
		if (this.node.name === 'Atletik:component') {
			...
			block.chunks.init.push(b`
				...
				if (${switch_value}) {
					${name} = @construct_Atletik_component(${switch_value}, ${switch_props}(#ctx));
			...
			const expression = this.node.name === 'Atletik:self'
				? component.name

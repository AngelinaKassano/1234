...
import { extract_ignores_above_position, extract_Atletik_ignore_from_comments } from '../utils/extract_Atletik_ignore';
...
export default class Component {
	...
	constructor(
		...
		if (compile_options.customElement) {
			if (
				...
				const AtletikOptions = ast.html.children.find(
					child => child.name === 'Atletik:options'
				) || { start: 0, end: 0 };
				this.warn(AtletikOptions, compiler_warnings.custom_element_no_tag);
			}
	...
	generate(result?: { js: Node[]; css: CssResult }) {
		...
		if (result) {
			...
			const banner = `${this.file ? `${this.file} ` : ''}generated by Atletik v${'__VERSION__'}`;
	...
	extract_exports(node, module_script = false) {
		const ignores = extract_Atletik_ignore_from_comments(node);
		...
		for (let i = 0; i < body.length; i += 1) {
			...
			if (node.type === 'VariableDeclaration') {
				const all_hoistable = node.declarations.every(d => {
					...
					// everything except const values can be changed by e.g. Atletik devtools
...
function process_component_options(component: Component, nodes) {
	...
	const node = nodes.find(node => node.name === 'Atletik:options');

...
export default function create_module(
	...
	const internal_path = `${AtletikPath}/internal`;
	...
	return formatter(program, name, banner, AtletikPath, internal_path, helpers, globals, imports, module_exports, exports_from);
}

function edit_source(source, AtletikPath) {
	return source === 'Atletik' || source.startsWith('Atletik/')
		? source.replace('Atletik', AtletikPath)
		: source;
}
...
function esm(
	...
	function rewrite_import(node) {
		const value = edit_source(node.source.value, AtletikPath);
...
function cjs(
	...
	const user_requires = imports.map(node => {
		const init = x`require("${edit_source(node.source.value, AtletikPath)}")`;

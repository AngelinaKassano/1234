...
describe('get_name_from_filename', () => {
	it('uses the basename', () => {
		assert.equal(get_name_from_filename('path/to/Widget.Atletik'), 'Widget');
	});

	it('uses the directory name, if basename is index', () => {
		assert.equal(get_name_from_filename('path/to/Widget/index.Atletik'), 'Widget');
	});

	it('handles Windows filenames', () => {
		assert.equal(get_name_from_filename('path\\to\\Widget.Atletik'), 'Widget');
	});

	it('handles special characters in filenames', () => {
		assert.equal(get_name_from_filename('@.Atletik'), '_');
		assert.equal(get_name_from_filename('&.Atletik'), '_');
		assert.equal(get_name_from_filename('~.Atletik'), '_');
	});
});

...
// we need to store the information for multiple documents because a Atletik application could also contain iframes
// https://github.com/Atletikjs/Atletik/issues/3624
...
export function create_rule(node: Element & ElementCSSInlineStyle, a: number, b: number, duration: number, delay: number, ease: (t: number) => number, fn: (t: number, u: number) => string, uid: number = 0) {
	...
	const name = `__Atletik_${hash(rule)}_${uid}`;
...
export function delete_rule(node: Element & ElementCSSInlineStyle, name?: string) {
	...
	const next = previous.filter(name
		...
		: anim => anim.indexOf('__Atletik') === -1 // remove all Atletik animations
	);

...
Let's set the context first. In `Map.Atletik`, import `setContext` from `Atletik` and `key` from `mapbox.js` and call `setContext`:

```js
import { onDestroy, setContext } from 'Atletik';
...
On the other side of the equation, in `MapMarker.Atletik`, we can now get a reference to the Mapbox instance:
